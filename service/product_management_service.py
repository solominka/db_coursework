...
class ProductManagementService:
    ...
    def create_product(self, request_body):
        ...
        def callee(session):
            ...
            client = self.__clientRepo.find_by_buid(buid=buid, tx=tx)
            if len(client) == 0:
            ...
            account_numbers = self.__id_generator.generate_account_numbers(product=request_body['product'], tx=tx)
        ...
        self.__ydb_pool.retry_operation_sync(callee)
        return agreement_id
    ...
    def close_product(self, request_body):
        def callee(session):
            ...
            agreement = self.__agreementRepo.find_by_id(tx=tx, id=agreement_id)
            if len(agreement) == 0:
            ...
            self.__agreementRepo.close_agreement(id=agreement_id, tx=tx)
            self.__accountRepo.close_accounts(agreement_id=agreement_id, tx=tx)
